...
export const GameBoard: FC<GameBoardProps> = ({
  ...
  // Безопасный массив игроков с защитой от undefined
  ...
  return (
    <div className="bg-[#1c082e] border border-purple-900/60 rounded-3xl p-3 sm:p-4 flex flex-col justify-between shadow-2xl w-full h-full space-y-2">
      {/* Верхний статус */}
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-purple-800/40 pb-2 px-1">
        <div className="flex items-center space-x-2">
          <span className="text-xs font-black text-amber-300">CASHFLOW</span>
          <span className="text-[11px] font-mono text-purple-300">| Малый круг</span>
        </div>
        ...
        <div className="flex items-center space-x-1.5 overflow-x-auto py-0.5 max-w-full">
          {safePlayers.map((p) => (
            <div
              key={p.id}
              className={`flex items-center space-x-1 px-2 py-0.5 rounded-lg text-[11px] border shrink-0 ${
                p.id === activePlayer.id
                  ? 'border-amber-400 bg-amber-400/20 text-amber-300 font-bold'
                  : 'border-slate-800 bg-slate-950/80 text-slate-400'
              }`}
            >
              <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: p.color?.hex || '#a855f7' }} />
              <span>{p.name}</span>
              <span className="text-[10px] opacity-75 font-mono">#{p.position}</span>
            </div>
          ))}
        </div>
      ...
      {/* SVG Canvas */}
      <div className="relative w-full flex-1 flex items-center justify-center min-h-[520px]">
        <svg
          viewBox="0 0 1200 750"
          preserveAspectRatio="xMidYMid meet"
          className="w-full h-full select-none drop-shadow-2xl"
        >
          {/* Скоростная дорожка (Периметр) */}
          <rect
            x="15"
            y="15"
            width="1170"
          ...
          {BOARD_TILES.map((tile, i) => {
            ...
            return (
              <g key={tile.id}>
                ...
                {/* Поворот системы координат по лучу сектора */}
