...
export const MarketModal: FC<MarketModalProps> = ({
  ...
  const matchingAssets = playerAssets.filter((asset) => {
    if (card.targetType === 'REAL_ESTATE' && asset.type === 'REAL_ESTATE') {
      if (cardSubtype) {
        return asset.title.toLowerCase().includes(cardSubtype.toLowerCase());
      }
      return true;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-amber-500/50 w-full max-w-lg rounded-3xl p-6 shadow-2xl space-y-4 text-slate-100 animate-in fade-in zoom-in-95 duration-200 max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-start border-b border-slate-800 pb-3">
          <div>
            ...
            <h3 className="text-lg font-black text-slate-100 mt-1">{card.title}</h3>
          </div>
          <span className="text-3xl">{isSplit ? '✂️' : '🏦'}</span>
        </div>
        ...
        {/* Сценарий 1: Сплит (дробление) акций */}
        {isSplit ? (
          <div className="space-y-3">
            <div className="bg-purple-950/30 border border-purple-800/60 p-3.5 rounded-2xl text-xs flex justify-between items-center">
          ...
          /* Сценарий 2: Выкуп активов покупателем */
          <>
            <div className="bg-amber-950/30 border border-amber-800/60 p-3.5 rounded-2xl flex justify-between items-center text-xs font-mono">
            ...
            <div className="space-y-2">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">
                Ваши активы под это предложение:
              </span>
                      ...
                      >
                          ...
                          <div className="text-[10px] text-slate-400 font-mono mt-0.5 flex items-center space-x-1">
                            {!isStock && mortgage > 0 && (
                              <>
                                ...
                                <Tooltip text={`При продаже банк автоматически списывает остаток долга (${mortgage.toLocaleString()} $), а остаток выплачивается вам на руки наличными.`} />
                              </>
                            )}
                            {isStock && <span>Количество: {asset.sharesCount} акций</span>}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-3 shrink-0">
                          ...
                          <button
                            ...
                            className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black px-3.5 py-2 rounded-xl text-xs transition cursor-pointer shadow-md shadow-emerald-500/20"
                          >
                            ПРОДАТЬ
                          </button>
                        </div>
